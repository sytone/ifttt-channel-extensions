...
function send_cross_platform_notification($application, $subject, $body = "") {
    return send_iphone_notification($application, $subject, $body) && send_android_notification($application, $subject, $body);
}

function send_iphone_notification($application, $subject, $body = "") {
    __log("Sending iPhone ('Prowl') notification");
    return send_push_notification("https://api.prowlapp.com/publicapi/add", PROWL_API_KEY, $application, $subject, $body);
}
...
function send_android_notification($application, $subject, $body = "") {
    ...
    return send_push_notification("https://www.notifymyandroid.com/publicapi/notify", NOTIFY_MY_ANDROID_API_KEY, $application, $subject, $body);
}
